...
cores_preenchimento1 <- c() # inicia um vetor vazio

for (i in 1:length(olimpiadas$ano)) {
  
  if(olimpiadas$prata[i] > 0) {
    
    cores_preenchimento1 <- c(cores_preenchimento1, "#A7A7AD")
    
  } else {
    cores_preenchimento1 <- c(cores_preenchimento1, "#CCCCCC")
  }
}
```
...
barplot(olimpiadas$total, names.arg = olimpiadas$ano, col=cores_preenchimento1, border=NA, xlab="Ano", ylab="Total de Medalhas")
```
...
cores_preenchimento2 <- c() # inicia um vetor vazio

for (i in 1:length(olimpiadas$ano)) {
  
  if(olimpiadas$bronze[i] > 0) {
    
    cores_preenchimento2 <- c(cores_preenchimento2, "#A77044")
    
  } else {
    cores_preenchimento2 <- c(cores_preenchimento2, "#CCCCCC")
  }
}
```

```{r}
barplot(olimpiadas$total, names.arg = olimpiadas$ano, col=cores_preenchimento2, border=NA, xlab="Ano", ylab="Total de Medalhas")
```
...
4. Analise, compare, pense e responda:
  ...
  Sim! Percebe-se que o Brasil ganhou cada vez mais medalhas ao longo do tempo.
  
  + O que você aprendeu com os gráficos?
  + Quais insights você obteve?
  + Que o Brasil ganhou medalhas bronze em praticamente todos os anos de olimpiadas que participou.Os anos 1956 e 1992, foram os únicos que o Brasil não ganhou medalhas Bronze.
